...
// Short resumé multilíngue (PT-BR / EN / ES / FR). Textos de interface em I18N;
// briefs de destaque/pesquisa/bolsas traduzidos em .i18n de cada objeto.
...
const REPOS = [
  ...
  { name: "Sistema Acadêmico", cat: "software", visibility: "privado", icon: "academic",
    ...
    i18n: { en: "Exclusive web app for Computer Engineering students at IFF to track curriculum, diary, grades, attendance, GPA/report card and planning. Fork of the evaluations standard, with diary, ementa, dependency graph and a Telegram-bot ingestion API. Deployed via Docker Compose + systemd.",
  ...
  { name: "Sistema de Avaliações", cat: "software", visibility: "privado", icon: "quiz",
    ...
    i18n: { en: "Base platform for exams/lists/assignments in PDF from a reusable question bank, with real LaTeX generation and multiple variants (random selection/order). Professor+admin core.",
  ...
  { name: "ReLaTeX", cat: "software", visibility: "privado", icon: "latex",
    ...
    i18n: { en: "Fork of Overleaf Community Edition with IFF identity: official document classes (ifftese/iffposter), blue theme, custom landing page and 10 extra editor buttons. Self-hosted via Docker (web+mongo+redis).",
  ...
  { name: "anomaly_detection", cat: "pesquisa", visibility: "privado", icon: "star",
    ...
    i18n: { en: "Analysis of stellar properties and chemical composition of nearby stars from GCNS observed by GALAH DR4 (Gaia DR3 + GALAH DR4). PIBIC/CNPq scholarship; co-authorship PUC Chile (Dr. M. L. L. Dantas, Dr. A. C. Soja).",
  ...
  { name: "SpectraViewer", cat: "pesquisa", visibility: "público", icon: "spectrum",
    ...
    i18n: { en: "Interactive viewer for GALAH DR4 spectra (4 bands, CNO/Alpha/Iron-peak reference lines, FITS reading, Teff/log(g)/[Fe/H] metadata).",
  ...
  { name: "research", cat: "pesquisa", visibility: "privado", icon: "book",
    ...
    i18n: { en: "Research attachments: arXiv article search by subject/keyword to compose readings.",
  ...
  { name: "Currículo (CV)", cat: "academico", visibility: "privado", icon: "document",
    ...
    i18n: { en: "CV in LaTeX (altacv class), multilingual (PT/EN/ES/FR) compiled via Makefile. Includes ORCID, Google Scholar and conference publications (CONFICT, SAB, SBPC).",
...
const BOLSAS = [
  { title: "Entendendo a Matéria Escura a partir de Choques ExtraGalácticos", icon: "darkmatter",
    ...
    i18n: { en: { title: "Understanding Dark Matter from ExtraGalactic Shocks", desc: "Junior Scientific Initiation on dark matter and extragalactic shocks.", kind: "Junior SI · CNPq" },
      es: { title: "Entendiendo la Materia Oscura a partir de Choques ExtraGalácticos", desc: "Iniciación Científica Júnior sobre materia oscura y choques extragalácticos.", kind: "IC Júnior · CNPq" },
      fr: { title: "Comprendre la Matière Noire à partir de Chocs ExtraGalactiques", desc: "Initiation Scientifique Junior sur la matière noire et les chocs extragalactiques.", kind: "SI Junior · CNPq" } } },
  { title: "MobFog no IFFMaker", icon: "fog",
    ...
    i18n: { en: { title: "MobFog at IFFMaker", desc: "IFF Scientific Initiation project (fog/edge computing).", kind: "SI · IFF" },
      es: { title: "MobFog en IFFMaker", desc: "Proyecto de Iniciación Científica del IFF (computación en niebla/edge).", kind: "IC · IFF" },
      fr: { title: "MobFog à l'IFFMaker", desc: "Projet d'Initiation Scientifique de l'IFF (informatique en brouillard/edge).", kind: "SI · IFF" } } },
  { title: "Simulando o Impacto de Satélites em Observações Astronômicas", icon: "satellite",
    ...
    i18n: { en: { title: "Simulating the Impact of Satellites on Astronomical Observations", desc: "Simulation of satellite-trail impact on astronomical images.", kind: "Volunteer · CNPq" },
      es: { title: "Simulando el Impacto de Satélites en Observaciones Astronómicas", desc: "Simulación del impacto de rastros de satélites en imágenes astronómicas.", kind: "Voluntario · CNPq" },
      fr: { title: "Simuler l'Impact des Satellites sur les Observations Astronomiques", desc: "Simulation de l'impact des traînées de satellites sur les images astronomiques.", kind: "Bénévole · CNPq" } } },
  { title: "Detecção de Anomalias em Estrelas da Via Láctea (Gaia + Surveys + ML)", icon: "star",
    ...
    i18n: { en: { title: "Anomaly Detection in Stars of the Milky Way (Gaia + Surveys + ML)", desc: "PIBIC/CNPq scholarship: machine learning on Gaia DR3 + GALAH DR4 + GCNS for galactic archaeology and stellar populations. Co-authorship PUC Chile.", kind: "SI · CNPq (PIBIC)" },
      es: { title: "Detección de Anomalias en Estrellas de la Vía Láctea (Gaia + Surveys + ML)", desc: "Beca PIBIC/CNPq: aprendizaje de máquina sobre Gaia DR3 + GALAH DR4 + GCNS para arqueología galáctica y poblaciones estelares. Coautoría PUC Chile.", kind: "IC · CNPq (PIBIC)" },
      fr: { title: "Détection d'Anomalies chez les Étoiles de la Voie Lactée (Gaia + Surveys + ML)", desc: "Bourse PIBIC/CNPq : apprentissage automatique sur Gaia DR3 + GALAH DR4 + GCNS pour l'archéologie galactique et les populations stellaires. Co-autoria PUC Chile.", kind: "SI · CNPq (PIBIC)" } } }
];
...
const CONTACTS = [
  ...
  { icon: "orcid", labelKey: "orcid", value: ORCID, href: "https://orcid.org/" + ORCID },
  { icon: "lattes", labelKey: "lattes", value: "6818168089966785", href: "http://lattes.cnpq.br/6818168089966785" },
  { icon: "instagram", labelKey: "instagram", value: "@" + INSTAGRAM, href: "https://instagram.com/" + INSTAGRAM },
  { icon: "globe", labelKey: "site", value: "www.phrandrade.com", href: "https://www.phrandrade.com/pt-br/" },
  { icon: "pin", labelKey: "location", value: "Bom Jesus do Itabapoana, RJ — Brasil", href: null }
];
...
const I18N = {
  pt: {
    ...
    hero: { eyebrow: "Engenharia de Computação · Instituto Federal Fluminense",
      ...
      badges: ["🛰️ PIBIC / CNPq","🏆 IAAC 2024 & 2025","📡 CCNA · CyberOps · IoT","🐧 Linux & Servidores","🔭 Astrofísica Computacional","🤖 Machine Learning","🪐 Gaia · GALAH","📄 ORCID · Scholar"],
      hobbyLabel: "Hobby: " },
    sections: {
      sobre: { title: "Trajetória entre código e estrelas", lead: "" },
      projetos: { title: "O que eu construo", lead: "Clique em qualquer cartão para revelar os detalhes técnicos." },
      trabalhos: { title: "Tudo o que eu faço no GitHub", lead: "Lista completa dos meus repositórios — públicos e privados — com um short brief de cada um. Clique em qualquer cartão para ver os detalhes." },
      pesquisa: { title: "Astronomia computacional", lead: "Projetos de pesquisa em astrofísica computacional. Clique para expandir." },
      bolsas: { title: "Iniciação Científica & Pesquisa", lead: "Projetos de bolsa e voluntariado em astrofísica computacional (extraídos do meu CV)." },
  ...
  en: {
    ...
    hero: { eyebrow: "Computer Engineering · Fluminense Federal Institute",
      ...
      badges: ["🛰️ PIBIC / CNPq","🏆 IAAC 2024 & 2025","📡 CCNA · CyberOps · IoT","🐧 Linux & Servers","🔭 Computational Astrophysics","🤖 Machine Learning","🪐 Gaia · GALAH","📄 ORCID · Scholar"],
      hobbyLabel: "Hobby: " },
    sections: {
      ...
      trabalhos: { title: "Everything I do on GitHub", lead: "Full list of my repositories — public and private — with a short brief of each. Click any card for details." },
      pesquisa: { title: "Computational astronomy", lead: "Computational astrophysics research projects. Click to expand." },
      bolsas: { title: "Scientific Initiation & Research", lead: "Scholarship and volunteer projects in computational astrophysics (from my CV)." },
  ...
  es: {
    ...
    hero: { eyebrow: "Ingeniería Informática · Instituto Federal Fluminense",
      ...
      badges: ["🛰️ PIBIC / CNPq","🏆 IAAC 2024 y 2025","📡 CCNA · CyberOps · IoT","🐧 Linux y Servidores","🔭 Astrofísica Computacional","🤖 Aprendizaje Automático","🪐 Gaia · GALAH","📄 ORCID · Scholar"],
      hobbyLabel: "Pasatiempo: " },
    sections: {
      sobre: { title: "Trayectoria entre código y estrellas", lead: "" },
      projetos: { title: "Lo que construyo", lead: "Haz clic en cualquier tarjeta para ver los detalles técnicos." },
      trabalhos: { title: "Todo lo que hago en GitHub", lead: "Lista completa de mis repositorios — públicos y privados — con un breve resumen de cada uno. Haz clic en cualquier tarjeta para ver detalles." },
      pesquisa: { title: "Astronomía computacional", lead: "Proyectos de investigación en astrofísica computacional. Haz clic para expandir." },
      bolsas: { title: "Iniciación Científica e Investigación", lead: "Proyectos de beca y voluntariado en astrofísica computacional (de mi CV)." },
  ...
  fr: {
    ...
    hero: { eyebrow: "Génie Informatique · Institut Fédéral Fluminense",
      ...
      badges: ["🛰️ PIBIC / CNPq","🏆 IAAC 2024 et 2025","📡 CCNA · CyberOps · IoT","🐧 Linux et Serveurs","🔭 Astrophysique Computationnelle","🤖 Apprentissage Automatique","🪐 Gaia · GALAH","📄 ORCID · Scholar"],
      hobbyLabel: "Loisir : " },
    sections: {
      sobre: { title: "Parcours entre code et étoiles", lead: "" },
      projetos: { title: "Ce que je construis", lead: "Cliquez sur une carte pour révéler les détails techniques." },
      trabalhos: { title: "Tout ce que je fais sur GitHub", lead: "Liste complète de mes dépôts — publics et privés — avec un court résumé de chacun. Cliquez sur une carte pour les détails." },
      pesquisa: { title: "Astronomie computationnelle", lead: "Projets de recherche en astrophysique computationnelle. Cliquez pour déployer." },
      bolsas: { title: "Initiation Scientifique et Recherche", lead: "Projets de bourse et bénévoles en astrophysique computationnelle (issus de mon CV)." },
